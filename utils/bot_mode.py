...
async def bulk_delete_handler(client: Client, message: Message):
    ...
    async def _poll():
        ...
        while True:
            ...
            tg = prog.get("telegram_deleted", 0)
            dr = prog.get("drive_deleted", 0)
            err = prog.get("errors", 0)

            def _bar(done, total, length=15):
                if total <= 0: return "░" * length
                filled = int(done / total * length)
                return "█" * filled + "░" * (length - filled)

            tg_pct = int(tg / total * 100) if total else 0
            dr_pct = int(dr / total * 100) if total else 0

            txt = (
                f"🗑️ **Bulk Delete**\n\n"
                f"**Status:** `{status}`\n\n"
                f"**Telegram messages**\n"
                f"`{_bar(tg, total)}` {tg_pct}%\n"
                f"`{tg:,}` / `{total:,}`\n\n"
                f"**Drive entries**\n"
                f"`{_bar(dr, total)}` {dr_pct}%\n"
                f"`{dr:,}` / `{total:,}`\n\n"
                f"**Errors:** `{err}`"
            )
            if txt != last_text:
