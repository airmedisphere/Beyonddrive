...
/file_ids - Show the range of Telegram IDs actually in your drive (use this before /bulk_delete)
...
async def search_handler(client: Client, message: Message):
    ...
    _drive_data = _dh.DRIVE_DATA

    if _drive_data is None:
        await message.reply_text("❌ Drive not initialized.")
        return
...
async def restricted_import_handler(client: Client, message: Message):
    ...
    logger.info(f"[DEBUG] /restricted_import HANDLER FIRED for chat {message.chat.id}")
    global BOT_MODE
...
async def bulk_delete_handler(client: Client, message: Message):
    ...
    logger.info(f"[DEBUG] /bulk_delete HANDLER FIRED for chat {message.chat.id}")
    if message.chat.id in _pending_requests:
        await message.reply_text(
            "I'm already waiting for your input. Please respond or /cancel."
        )
...
# ═══════════════════════════════════════════════════════════════════════════════
# /file_ids — diagnostic: show the range of Telegram message IDs in your drive
# ═══════════════════════════════════════════════════════════════════════════════
@main_bot.on_message(
    filters.command("file_ids")
    & filters.private
    & filters.user(config.TELEGRAM_ADMIN_IDS),
)
async def file_ids_handler(client: Client, message: Message):
    """Show the range of file_ids (Telegram message IDs) actually stored in the drive."""
    from utils.directoryHandler import DRIVE_DATA
    ...
    storage_ids = []      # files stored in your STORAGE_CHANNEL
    fast_ids = []         # fast-imported files (point to other channels)
    fast_channels = {}    # source_channel -> count
        ...
        for item in folder.contents.values():
            if getattr(item, "type", None) == "file":
                ...
                if isinstance(fid, int):
                    if getattr(item, "is_fast_import", False):
                        fast_ids.append(fid)
                        ch = getattr(item, "source_channel", "unknown")
                        fast_channels[ch] = fast_channels.get(ch, 0) + 1
                    else:
                        storage_ids.append(fid)
            elif getattr(item, "type", None) == "folder":
                walk(item)

    walk(_drive_data.get_directory("/"))

    lines = ["📊 **File ID Inventory**\n"]

    if storage_ids:
        lines.append(f"**STORAGE_CHANNEL files:** {len(storage_ids):,}")
        lines.append(f"  Min ID: `{min(storage_ids)}`")
        lines.append(f"  Max ID: `{max(storage_ids)}`")
        lines.append(f"  Sample: `{', '.join(str(x) for x in sorted(storage_ids)[-5:])}`")
        lines.append("")
        lines.append("✅ Use /bulk_delete with links from your STORAGE_CHANNEL "
                     "(IDs between these ranges).")
    else:
        lines.append("⚠️ No regular files (all fast-imported).")

    if fast_ids:
        lines.append("")
        lines.append(f"**Fast-imported files:** {len(fast_ids):,}")
        for ch, count in list(fast_channels.items())[:5]:
            lines.append(f"  Source `{ch}`: {count:,} files")
        lines.append("")
        lines.append("⚠️ Fast-import files **cannot** be deleted by /bulk_delete "
...
async def _handle_all_messages(client: Client, message: Message):
    ...
    """
    logger.info(f"[DEBUG] catch-all _handle_all_messages got: {message.text[:50] if message.text else '?'}")
    chat_id = message.chat.id
