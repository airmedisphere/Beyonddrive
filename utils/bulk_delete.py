...
async def execute_delete(
    ...
    # ── Phase 2: remove drive entries (FAST — no per-file save, one save at the end) ──
    BULK_DELETE_PROGRESS[delete_id]["status"] = "removing from drive"
    drive_data = _drive()

    # Pre-build a map: file.id (drive ID) -> (parent_folder_data, file_id_in_folder)
    # so we don't walk the whole tree for every deletion
    file_map = {}
    def _build_map(folder):
        for item in list(folder.contents.values()):
            if getattr(item, "type", None) == "file":
                file_map[item.id] = folder
            elif getattr(item, "type", None) == "folder":
                _build_map(item)
    _build_map(drive_data.contents["/"])

    for m in matches:
        if delete_id in BULK_DELETE_CANCEL:
            break
        try:
            parent_folder = file_map.get(m["id"])
            if parent_folder and m["id"] in parent_folder.contents:
                # Direct dict deletion — bypass delete_file_folder() to avoid 174 saves
                del parent_folder.contents[m["id"]]
    ...
    # ONE save at the end (not 174)
    BULK_DELETE_PROGRESS[delete_id]["status"] = "saving drive"
    try:
        drive_data.save()
    except Exception as e:
        logger.error(f"[BulkDelete] Final save failed: {e}")
