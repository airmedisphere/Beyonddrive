...
document.addEventListener('DOMContentLoaded', function() {
    ...
    bulkMove.addEventListener('click', function() {
        ...
        alert('Bulk Move is not yet implemented. Use the "..." menu on individual items to move them, or Archive multiple items.');
    });
    ...
    bulkCopy.addEventListener('click', function() {
        ...
        alert('Bulk Copy is not yet implemented. Use the "..." menu on individual items to copy them.');
    });
    ...
    async function bulkArchiveOrDelete(archiveOnly) {
        ...
        console.log('[BulkAction] selectedItems contains', total, 'IDs');
        ...
        for (const id of items) {
            ...
            tasks.push({ id, path: itemPath + '/' + id, name: itemEl.getAttribute('data-name') || id });
        }

        console.log('[BulkAction] resolved', tasks.length, 'tasks,', missing.length, 'missing');
        if (missing.length) console.log('[BulkAction] missing IDs:', missing.slice(0, 5), '...');

        const total_real = tasks.length;
        if (total_real === 0) {
            alert(`Could not resolve item paths for any of the ${total} selected items.\n\nThis usually means the items are not in the current view. Scroll to load them or reload the page.`);
            return;
        }

        // Confirm the actual count we can act on
        if (total_real < total) {
            if (!confirm(`Only ${total_real} of ${total} selected items are loaded and will be processed.\n\nProceed?`)) {
                return;
            }
        }

        // Show progress indicator on the toolbar
        let done = 0, errors = 0;
        const updateStatus = () => {
        ...
        // Concurrency 4 — enough to be fast, not so many we trigger rate limits
        const CONCURRENCY = 4;
        let cursor = 0;
        async function worker(wid) {
            while (cursor < tasks.length) {
                ...
                try {
                    ...
                    console.log(`[BulkAction] worker ${wid}: ${t.name} →`, resp);
                    if (resp && resp.status === 'ok') {
                        done++;
                    } else {
                        errors++;
                        console.warn('[BulkAction] failed:', t.name, resp);
                    }
                } catch (e) {
                    errors++;
                    console.error('[BulkAction] exception:', t.name, e);
                }
                updateStatus();
            }
        }
